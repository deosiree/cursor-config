...
const mfaMethodText = computed(() => {
  if (mfaMethodCode.value === "MFA_METHOD_NONE") return t("关闭（无需验证码）");
  if (mfaMethodCode.value === "MFA_METHOD_SMS") return t("短信验证码");
  if (mfaMethodCode.value === "MFA_METHOD_EMAIL") return t("邮箱验证码");
  if (mfaMethodCode.value === "MFA_METHOD_BOTH") return t("短信/邮箱验证码（二选一）");
  return t("未配置");
});
const mfaCodePlaceholder = computed(() =>
  effectiveMfaChannel.value === "sms" ? t("请输入短信验证码") : t("请输入邮箱验证码")
);
const mfaHintText = computed(() => {
  if (!requiresMfa.value) return t("当前登录配置无需 MFA。");
  if (!isMfaChallengeActive.value) return t("请先点击“登录”完成账号密码校验，再发送 MFA 验证码。");

  const channelText = effectiveMfaChannel.value === "sms" ? t("短信") : t("邮箱");
  const masked = effectiveMfaChannel.value === "sms" ? mfaMaskedPhone.value : mfaMaskedEmail.value;
  const prefix = t("验证码将发送到");

  return masked ? prefix + channelText + "：" + masked : prefix + channelText + "。";
});
...
async function sendMfaChallengeCode() {
  ...
  try {
    ...
    showNotification(
      effectiveMfaChannel.value === "sms" ? t("短信验证码已发送") : t("邮箱验证码已发送"),
      { type: "success" }
    );
