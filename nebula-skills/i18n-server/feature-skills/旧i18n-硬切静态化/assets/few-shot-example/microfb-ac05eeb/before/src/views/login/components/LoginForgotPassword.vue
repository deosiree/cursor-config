<template>
  <ForgotVerifyStep
    ...
    :form="form"
    :rules="verifyRules"
    :captcha-placeholder="t('login.captchaCode')"
...
const resetRules = computed<FormRules>(() => ({
  password: [
    requiredRule(MSG.passwordRequired, "blur"),
    {
      min: 6,
  ...
  confirmPassword: createConfirmPasswordRulesWithMin(() => form.password),
}));
...
async function handleSendCode() {
  ...
  try {
    ...
    showNotification(result.message || "验证码已发送，请注意查收", { type: "success" });
  } catch {}
}
...
async function handleConfirmReset() {
  ...
  try {
    ...
    showNotification(result.message || "密码重置成功，请使用新密码登录", { type: "success" });
