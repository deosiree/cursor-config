...
httpRequest.interceptors.response.use(
  (response: AxiosResponse<any>) => {
    ...
    // 业务失败
    if (!config.silent) {
      showNotification(message || t("业务错误"), { type: "error" });
    }

    return Promise.reject({
      type: "business",
      error: new Error(message || t("业务错误")),
      response,
    });
  },
  async (error) => {
    ...
    if (!response) {
      ...
      showNotification(t("网络连接失败，请检查网络设置"), { type: "error" });
      return Promise.reject({ type: "network", error, config });
    }
    // 401 Unauthorized 错误处理
    if (response.status === 401) {
      await redirectToLogin(t("登录状态已失效，请重新登录"));
 ...
 * 跳转登录页
 */
async function redirectToLogin(message: string = t("请重新登录")) {
  try {
    showNotification(message, { title: t("提示"), type: "warning" });
