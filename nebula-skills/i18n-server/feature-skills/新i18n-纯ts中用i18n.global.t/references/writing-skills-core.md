...
- 本节点重点讨论纯 TS 文件直接依赖全局 i18n 实例；但当组件只是消费这类工厂提前求值后的结果时，也承接“组件侧显式订阅 `locale` 以触发重建”的联动变体。

## 为什么独立成新节点

- `request.ts`、`util.ts`、`helper.ts` 等文件没有 `setup()` 上下文，无法直接套用组件内 `t()` 的消费模式。
- 这类文件通常需要显式 `import i18n`，并在文件内建立局部 `t` 包装或直接调用 `i18n.global.t(...)`。
- `formRules.ts` 这类工厂若在执行时就把文案翻译成字符串，组件侧即使不直接调用 `i18n.global.t(...)`，也仍然需要显式读取 `i18n.global.locale.value` 才能在切语言后重建结果。
...
- 不把某个仓库的路径布局误认为唯一做法，而是提炼“纯 TS 全局实例消费”和“组件侧 locale 订阅重建”的功能边界。

## 何时补新的 few-shot

- 同一功能在另一仓库出现了新的稳定成功提交。
- 当前 few-shot 不能覆盖明显不同的纯 TS 文件角色，例如请求层、路由辅助层、表单工具层，或“纯 TS 工厂 + 组件 computed 重建”的联动模式。
