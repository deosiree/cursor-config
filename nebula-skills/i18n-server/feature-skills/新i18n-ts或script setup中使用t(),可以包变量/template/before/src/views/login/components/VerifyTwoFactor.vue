    ...
    >
      <el-form-item>
        <el-input
          ...
          :placeholder="verifyType === 'mobile' ? '手机号' : '邮箱'"
          disabled
        >
          <template #prefix>
      ...
      <el-form-item prop="authCode">
        <div class="form-inline">
          ...
          <div class="send-code">
            ...
            >
              {{
                expiresCountdown > 0 ? `有效期还剩 ${expiresCountdown}s` : "验证码已过期，请重发"
              }}
            </div>
          </div>
        </div>
  ...
  authCode: [
    {
      required: true,
      trigger: "blur",
      message: verifyType.value === "mobile" ? "请输入手机验证码" : "请输入邮箱验证码",
    },
  ],
}));
...
async function sendVerificationCode() {
  if (!maskedContact.value) {
    showNotification(verifyType.value === "mobile" ? "手机号不能为空" : "邮箱不能为空", {
      type: "warning",
    });
    return;
  }
  ...
  try {
    ...
    showNotification(verifyType.value === "mobile" ? "手机验证码已发送" : "邮箱验证码已发送", {
      type: "success",
    });
...
async function handleVerifySubmit() {
  try {
    ...
    showNotification("验证成功", { type: "success" });
  } catch (error) {
