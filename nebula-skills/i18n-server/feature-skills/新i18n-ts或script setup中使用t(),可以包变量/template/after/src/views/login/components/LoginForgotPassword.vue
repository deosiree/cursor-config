...
const {
  ...
  bindResetOnChange,
} = useCaptchaSendSuccess(); // 验证码发送状态管理
const {
...
onUnmounted(() => {
  clearCountdown(countdown, countdownTimer); // 清除倒计时
});

bindResetOnChange(() => form.account.trim()); // 账号输入框
bindResetOnChange(() => form.captchaAnswer.trim()); // 验证码输入框
