 ...
 * @param locale - 目标语言
 * @returns 指定 locale 下的展示文案
 */
export function resolveI18nTextAtLocale(title: unknown, locale: string) {
  if (typeof title !== "string" || !title) {
    return "";
  }

  if (parseI18nJsonText(title)) {
    const localized = resolveI18nJsonText(title, locale);
    if (localized) return localized;
  }

  const hasKey = i18n.global.te(title, locale);
  if (hasKey) {
    return i18n.global.t(title, {}, { locale });
  }
  return title;
}

/**
 * 按当前 global locale 解析展示文案。
 */
export function resolveI18nText(title: unknown) {
  return resolveI18nTextAtLocale(title, i18n.global.locale.value);
}
