...
type MenuCacheNode = {
  name?: unknown;
  menuName?: string;
  children?: MenuCacheNode[];
};

/**
 * 写入缓存前：保留 wire name，重写 menuName 为当前 locale 展示文案。
 */
export function projectMenuTreeForCache<T extends MenuCacheNode>(
  nodes: T[],
  ...
  return nodes.map((node) => ({
    ...
    children:
      node.children && node.children.length > 0
        ? projectMenuTreeForCache(node.children as T[], locale)
        : node.children,
  }));
}
...
export function writeMenuCache<T = any[]>(nodes: T): T {
  ...
  const projected = projectMenuTreeForCache(
    normalized as unknown as MenuCacheNode[]
  ) as unknown as T;
  localStorage.setItem(MENU_LIST_STORAGE_KEY, JSON.stringify(projected));
  menuVersionRef.value = computeVersion(projected);
  bumpTick();
