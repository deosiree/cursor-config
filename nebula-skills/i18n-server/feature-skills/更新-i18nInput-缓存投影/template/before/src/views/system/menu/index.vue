                  ...
                  <template #default="scope">
                    <div class="menu-name-cell">
                      ...
                      <span class="menu-name-text">
                        {{ resolveI18nJsonText(scope.row.name, locale, scope.row.menuName) }}
                      </span>
                    </div>
                  </template>
                </el-table-column>
...
const menuTabs = computed<PageTabShellTabItem[]>(() => {
  ...
  return menuTableData.value
    ...
    .map((menu) => ({
      key: menu.id || "",
      label: resolveI18nJsonText(menu.name, locale.value, menu.menuName || ""),
    }));
});
...
async function fetchMenuList() {
  ...
  menuTableData.value = filterMenusByKeyword(
    menuPageSource.value,
...
watch(locale, () => {
  if (menuPageSource.value.length === 0) return;
  menuTableData.value = filterMenusByKeyword(
    menuPageSource.value,
    queryParams.keywords || "",
    locale.value
