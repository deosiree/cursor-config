  ...
  >
    <div id="bindDevice" v-loading="loading" class="transfer-container">
      <customTransfer
        ...
        filter-placeholder="请输入设备名称、设备描述、机器码、租户、状态"
          ...
          <div class="transfer-item">
            ...
            <div class="transfer-item__desc">
              <el-tag
                ...
                disable-transitions
              >
                {{ DEVICE_STATUS_CONFIG[option.device.status].label }}
              </el-tag>
              <span v-else>-</span>
            </div>
          </div>
...
const transferTitles = computed<[string, string]>(() => {
  return [`待绑定设备 (${unboundTotal.value})`, `已绑定设备 (${boundTotal.value})`];
});
...
async function loadUnboundDeviceList() {
  ...
  try {
    ...
    showNotification("加载未绑定设备列表失败", { type: "error" });
...
async function loadBoundDeviceList() {
  ...
  try {
    ...
    showNotification("加载已绑定设备列表失败", { type: "error" });
...
async function loadDeviceList() {
  ...
  try {
    ...
    showNotification("加载设备列表失败", { type: "error" });
    unboundDeviceList.value = [];
    boundDeviceList.value = [];
    transferData.value = [];
    unboundTotal.value = 0;
...
function buildTransferData() {
  ...
  boundDeviceList.value.forEach((device) => {
    ...
    data.push({
      key: `bound-${device.id}`,
      label: device.deviceName || device.deviceDesc || device.machineCode || String(device.id),
      desc: `机器码: ${device.machineCode} | 状态: ${statusLabel}`,
      device,
      type: "bound",
    });
    // 已绑定的设备默认在右侧（选中状态）
...
async function handleSubmit() {
  ...
  try {
    ...
    if (res) {
      showNotification("操作成功", { type: "success" });
    }
    handleClose();
    emit("success");
  } catch (error) {
