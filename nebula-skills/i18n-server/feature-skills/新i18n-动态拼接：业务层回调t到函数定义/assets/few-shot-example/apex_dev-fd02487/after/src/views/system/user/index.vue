...
const resetRules = computed(() => ({ password: createPasswordWithMin6Rules(t) }));
...
const rules = computed(() => ({
  ...
  roleIds: createRoleIdsRules(t),
  password: createFieldPolicy.value.requirePassword ? createPasswordRules(t) : [],
  confirmPassword: createFieldPolicy.value.requirePassword
    ? createConfirmPasswordRules(t, () => formData.password ?? "")
    : [],
  email: [{ required: true, message: t("邮箱不能为空"), trigger: "blur" }, ...createEmailRules(t)],
  phone: createPhoneRules(t),
  roleId: createRoleIdRules(t),
}));
