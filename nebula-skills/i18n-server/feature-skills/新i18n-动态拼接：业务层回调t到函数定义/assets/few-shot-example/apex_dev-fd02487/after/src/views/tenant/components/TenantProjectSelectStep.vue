<template>
  <div class="tenant-project-step">
    <div class="tenant-project-step__title">{{ $t("项目列表") }}</div>
    <div v-if="treeData.length" class="project-tree-wrapper">
      <div class="project-tree-header">
        ...
        <span class="project-tree-header__index">{{ $t("序号") }}</span>
        <span class="project-tree-header__name">{{ $t("项目名称") }}</span>
        <span class="project-tree-header__description">{{ $t("描述") }}</span>
      </div>
      <el-tree
        ref="treeRef"
        :data="treeData"
    ...
    <el-empty v-else :image-size="80" :description="$t('暂无可选项目')" />
  </div>
</template>
