<template>
  ...
  >
    ...
    <el-form-item v-if="!readonlyMode && !model.id" :label="$t('激活方式')">
      <el-select v-model="activationMethod" :placeholder="$t('请选择激活方式')">
        <el-option
    ...
    <el-form-item v-if="showPasswordFields && !readonlyMode && !model.id" prop="password">
      <template #label>
        <div class="inline-flex items-center gap-1">
          <span>{{ $t("密码") }}</span>
          <el-tooltip placement="top" :content="$t('密码为必填项')">
      ...
      <el-input
        ...
        :placeholder="model.id ? $t('编辑时不可修改密码') : $t('请输入用户密码')"
        type="password"
        show-password
        maxlength="255"
        clearable
    ...
    <!-- 角色 -->
    <el-form-item v-if="showRole" :label="$t('角色')" prop="roleId">
      <el-select
    ...
    <!-- 所属租户 -->
    <el-form-item v-if="showTenant" :label="$t('所属租户')" prop="tenantId">
      <el-select
        v-model="model.tenantId"
        :placeholder="$t('请选择所属租户')"
        clearable
        :disabled="readonlyMode"
      >
        <el-option
