...
export function resolveMfaHintText(params: {
  ...
  const { requiresMfa, isMfaChallengeActive, channel, maskedPhone = "", maskedEmail = "" } = params;
  if (!requiresMfa) return "当前登录配置无需 MFA。";
  if (!isMfaChallengeActive) return "请先点击“登录”完成账号密码校验，再发送 MFA 验证码。";

  const channelText = getMfaChannelLabel(channel);
  const masked = channel === mfaChannel.sms ? maskedPhone : maskedEmail;
  return masked ? `验证码将发送到${channelText}：${masked}` : `验证码将发送到${channelText}。`;
}
