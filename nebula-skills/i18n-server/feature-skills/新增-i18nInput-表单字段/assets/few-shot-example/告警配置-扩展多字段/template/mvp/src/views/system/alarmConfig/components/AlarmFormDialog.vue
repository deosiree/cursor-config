    ...
    >
      ...
      <template v-if="formData.type === 'group'">
        ...
        <el-form-item :label="$t('告警组描述')">
          ...
          >
            <template #append>
              <I18nInput
                v-model:i18n-data="formData.description"
                :source-value="descriptionValue"
              />
            </template>
          </el-input>
        </el-form-item>
      ...
      <template v-else>
        ...
        <el-form-item :label="$t('告警模板')">
          ...
          >
            <template #append>
              <I18nInput v-model:i18n-data="formData.template" :source-value="templateValue" />
            </template>
          </el-input>
        </el-form-item>
      </template>
...
const descriptionValue = computed({
  get: () => String(cloneI18nData(formData.description)[getCurrentLocaleKey()] || ""),
  set: (value: string) => {
    formData.description = updateLanguageValue(formData.description, value);
  },
});

const templateValue = computed({
  get: () => String(cloneI18nData(formData.template)[getCurrentLocaleKey()] || ""),
  set: (value: string) => {
    formData.template = updateLanguageValue(formData.template, value);
  },
});
...
const handleSubmit = async () => {
  ...
  const namePayload = stringifyI18nData(formData.name);
  const descriptionPayload = stringifyI18nData(formData.description);
  const templatePayload = stringifyI18nData(formData.template);
...
const open = (type: "add" | "edit", row?: any, context?: "toolbar" | "table") => {
  ...
  if (type === "add") {
    ...
    formData.name = parseI18nData(row.rawName ?? row.name);
    formData.code = row.code;
    formData.description = parseI18nData(row.rawDescription ?? row.description);
    formData.level = row.level;
    formData.template = parseI18nData(row.rawTemplate ?? row.template);
    if (!row.isGroup) {
