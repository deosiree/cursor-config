...
// 在模块初始化时获取一次 appStore，用于语言等配置同步
...
export function handleGlobalStateChange(state: GlobalState, prev: GlobalState) {
  ...
  if (state.language && state.language !== appStore.language) {
    appStore.changeLanguage(state.language);
    // 更新 vue-i18n 全局 locale
    // @ts-expect-error vue-i18n 4.x 全局模式下 locale 为 ref
