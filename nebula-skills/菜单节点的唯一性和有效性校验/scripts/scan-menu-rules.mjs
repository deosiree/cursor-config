...
function printHelp() {
  console.log(`scan-menu-rules — 只读扫描菜单是否符合《前端的表单校验规则》（不推送）
...
function summarize(hits) {
  ...
  console.log(`\n合计违规 ${hits.length} 条`);
  console.log(`
解读建议（对照《前端的表单校验规则》；不以 seccenter 仓库为准）：
  ...
  page.combo       → type=page 的 route_path+params 全库唯一
`);
}
