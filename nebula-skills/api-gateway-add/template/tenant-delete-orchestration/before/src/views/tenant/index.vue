      ...
      <BaseListToolbar :title="$t('租户列表')">
        <template #filters>
          <el-input
            ...
            suffix-icon="search"
            :placeholder="$t('请输入租户名、联系人、手机号（关键词）')"
            clearable
            class="search-input"
            maxlength="20"
...
const ownerRules = computed(() => ({
  userName: createUserNameRules(t),
  password: ownerFieldPolicy.value.requirePassword ? createPasswordRules(t) : [],
  confirmPassword: ownerFieldPolicy.value.requirePassword
    ? createConfirmPasswordRules(t, () => createOwnerForm.password)
    : [],
  email: [{ required: true, message: t("邮箱不能为空"), trigger: "blur" }, ...createEmailRules(t)],
  phone: createPhoneRules(t),
}));
...
async function handleOpenProjectDialog(row: TenantTableRowModel) {
  ...
  try {
    const [options, projectsRes, tenantDevicesRes] = await Promise.all([
    ...
    const projectBindings = (projectsRes.projects ?? []).filter((project) =>
      Boolean(project.projectId)
    );
    const selectedProjectIds = Array.from(
      new Set([
        ...(projectsRes.projectIds ?? []),
        ...projectBindings.map((project) => String(project.projectId)),
      ])
    );
    ...
    projectDialogSelectedIds.value = selectedProjectIds; // 选中的项目ID
    projectDialogInitialBindings.value = nextBindings;
...
async function handleDelete(id?: string) {
  ...
  ElMessageBox.confirm(t("确定删除租户?"), t("警告"), {
