...
> 本节约定 **语义口径**：正文是对「本批变更」的压缩，不是产品文案或泛泛架构介绍；执行方应结合 `scan_changes`、S6 的 `map_notes`、会话与外部文档中的对比材料撰写。以下 **不出现** 具体业务字段或仓库内命名示例。

1. **`【摘要】`**：用一两句话概括 **相对基准的改动轮廓**（改了什么层：契约、状态、路由、UI 等），读者能指向「去看本批文件/diff」而非仅读到愿景。
2. **`定义`**：**必须**交代 **变更前** 与 **变更后** 的差异轮廓（实现方式、数据形态、职责边界之一即可）；若本批为演进而非绿场，**禁止**用「全新/整套新增」类表述 **替代** 对旧形态的描述。
3. **`问题`**：说明 **旧形态为何不足以达成当前目标**（一致性、契约对齐、可维护性、可观测性等），避免空洞的「缺少某某能力」而无可对照事实。
4. **`解决`**：描述与本批 `plan` 文件清单 **语义可对上** 的落点（模块层级、归一化/适配层、调用链调整等），不要求罗列所有路径，但不得写与 scan 明显无关的「假动作」。
5. **`价值`**：偏 **审查与发布说明**——使阅读者对照上一版本能建立正确心智（**演进**而非误读为突变）；可点明可回滚边界与验证方式（测试/手工步骤）若本批已有。
6. **禁止的无据断言**：在 **默认基线 `HEAD`** 下，对已跟踪文件的 `M/D/R` 等变更，不得使用「原先不存在」「从零开始」等 **与路径状态矛盾** 的措辞；对纯新增路径可使用「新增」类表述但须与 `scan_changes` 一致。若叙事目标是 **跨分支** 而缺少该基线的核对材料，不得断言远端形态，应中性表述并服从 S6 的置信度结论。
7. **对比基**：**默认**为当前分支 **`HEAD`**（最近一次提交），即「改前」= 已提交树，「改后」= 相对 `HEAD` 的未提交变更（与 S3 扫描一致）。若用户在 START 或 S4 中提供 **其它对比分支/合并基** 或 diff 摘要，S8 须与之对齐；跨分支未提供材料时仍须输出四段式，但不得伪造「改前」细节。
