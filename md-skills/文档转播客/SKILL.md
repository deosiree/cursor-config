---
name: 文档转博客
description: 创建型：中文技术MD→U型搭档播客听记稿（非静态博文）。生成前读 template/mvp 或 snapshot；交付朗读稿+可选MP3/SRT/详细解答。触发词：文档转博客、播客脚本、可朗读、双人播客、场景钩子、快问快答、MP3字幕、听记稿。
---

# 文档转博客（技术 MD → U 型搭档播客）

> Skill ID：`文档转博客`（原 `tech-doc-to-podcast`）。目录：`.cursor/md-skills/文档转博客/`

## 命名说明（必读）

| 用户说法 | 本 skill 实际产出 | 不做 |
| --- | --- | --- |
| 「转博客」「转播客」 | **双人听记对话稿**（可朗读）+ 可选 MP3/SRT/书面详细解答 | 纯静态技术博文、单人旁白 |
...
纯静态技术博客（非对话体）→ 先确认是否改做听记稿；非 `.md` → 停止；非中文 → TTS 仅 zh-CN。
