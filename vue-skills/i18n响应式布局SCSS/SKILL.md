---
name: i18n响应式布局SCSS
description: locale-layout 侧栏/表单/PageTabShell Tab sm-md-lg-xl 档位与 SCSS 撑满。触发：切换语言布局宽、sidebarWidth、tabLabelMaxWidth、menu.scss 200px 锁宽。先路由扩展已有/新建 MVP/修 SCSS/Tab 宽，勿用于纯 i18n 翻译。
---
...
**TL;DR 路由**：有 locale-layout → **扩展**；没有 → **新建 MVP**；容器已宽 menu 仍窄 → **修 SCSS**；PageTabShell Tab 叠层/不随语言 → **扩展 tabLabelMaxWidth + 项宽联动**。

## Agent 执行步骤（按序）

1. **读 RED 五问**（下节）与目标仓 `locale-layout.ts` / 侧栏模板 / `menu.scss` / `PageTabShell` 绑定
...
5. PageTabShell 只改 `tab-label-max-width` 但项仍 112px → 见 [`扩展-PageTabShell-tabLabelMaxWidth`](feature-skills/扩展-PageTabShell-tabLabelMaxWidth/SKILL.md)
6. 档位语义见 [`references/responsive-breakpoints.md`](references/responsive-breakpoints.md)（Element Plus + Bootstrap 约定）

对照样本：

- apex 扩展侧栏：[`template/before`](template/before/apex-dev/) vs [`template/after`](template/after/apex-dev/)
- apex PageTabShell Tab：[`menu.menu-tab`](template/before/apex-dev/src/views/system/menu.menu-tab.fragment.vue) / [`securityConfig.tab`](template/before/apex-dev/src/views/system/securityConfig.tab.fragment.vue) before|after
...
| **PageTabShell Tab** | `tab-label-max-width` 写死 / 英文 Tab 重叠 | [`feature-skills/扩展-PageTabShell-tabLabelMaxWidth`](feature-skills/扩展-PageTabShell-tabLabelMaxWidth/SKILL.md) |
| **microfb 全量** | 无插件 + 侧栏 + menu 锁宽 | **新建 → SCSS 修复** |

判定口诀：**有 locale-layout 就扩展；没有就 MVP；宽了还窄就修 SCSS；Tab 叠了绑 tabLabelMaxWidth。**
...
| 英文 Tab 叠到下一项 | 绑 `$localeLayout.tabLabelMaxWidth.md`；确认 PageTabShell `calc(label+3.5rem)` | [`扩展-PageTabShell-tabLabelMaxWidth`](feature-skills/扩展-PageTabShell-tabLabelMaxWidth/SKILL.md) |
| 安全配置 Tab 不随语言 | 仍写死 `200px` 或未绑 `$localeLayout` | 同上；无齿轮设 `show-tab-actions=false` |

## GREEN 验收

1. 新布局字段为 `Record<LayoutSize, string>`，与 `formLabel` 同构
2. 模板默认 `$localeLayout.{field}.md`（侧栏：`sidebarWidth.md`；Tab：`tabLabelMaxWidth.md`）
...
6. `vue-tsc` 无新增错误
7. PageTabShell：`showTabActions` 时项宽随 `tabLabelMaxWidth`；无操作区则 `show-tab-actions=false`
...
```text
使用 $i18n响应式布局SCSS：菜单管理英文 Tab 与运维中心重叠，扩展 tabLabelMaxWidth 并绑 PageTabShell；安全配置去掉写死 200px。
```
...
- [`assets/few-shot-example/pagetab-tablabel-session.md`](assets/few-shot-example/pagetab-tablabel-session.md) — PageTabShell 实跑摘要
...
- 勿只改 `tab-label-max-width` 而不确认 PageTabShell 项总宽（`showTabActions` 时须 `calc(label+3.5rem)`）
