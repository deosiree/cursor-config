  ...
  <div class="tenant-table h-full">
    <div ref="tableWrapperRef" class="table-wrapper">
      ...
      >
        ...
        <el-table-column :label="$t('绉熸埛鍚?)" prop="tenantName" width="auto" />
        <el-table-column :label="$t('鑱旂郴浜?)" prop="contactName" width="auto" />
        <el-table-column :label="$t('鎵嬫満鍙?)" align="center" prop="contactPhone" width="auto" />
        <el-table-column :label="$t('閭')" align="center" prop="contactEmail" min-width="170" />
        <el-table-column :label="$t('鐘舵€?)" align="center" prop="status" width="90">
        ...
        <el-table-column :label="$t('鍒版湡鏃堕棿')" align="center" prop="expireAt" min-width="160">
          <template #default="{ row }">
            <span :class="$t(getExpireTextClass(row))">
              {{ row.expireAt ? formatDateTime(row.expireAt) : $t("姘镐箙") }}
            </span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('鍒涘缓鏃堕棿')" align="center" prop="createdAt" min-width="160">
          <template #default="{ row }">
...
/**
 * 澶勭悊閫変腑椤瑰彉鍖? * @param selection 閫変腑鐨勭鎴峰垪琛? */
