  ...
  <div class="app-container user-manage">
    <el-card shadow="hover" class="bg-white bottom-container data-table h-full">
      ...
      <div class="user-list-page__body">
        <!-- 用户表格 -->
        <!-- 告警属性 @subscribe="handleOpenSubscribe" -->
        <UserTable
          :data="pageData"
          :loading="loading"
...
import {
  ...
  createConfirmPasswordRulesWithMin,
  createEmailRules,
  createPasswordRules,
...
const resetRules = computed(() => ({
  password: createPasswordWithMin6Rules(),
  confirmPassword: createConfirmPasswordRulesWithMin(() => resetForm.password ?? ""),
}));
...
const rules = computed(() => ({
  ...
  roleIds: createRoleIdsRules(),
  password: createFieldPolicy.value.requirePassword ? createPasswordRules() : [],
