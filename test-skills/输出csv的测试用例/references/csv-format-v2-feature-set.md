# CSV 格式 v2：功能集合 + 用例结果（alarm_.csv 风格）
...
|------|-----------------------------------|----------------------------------|
| 功能集合 | **强制留空** | **必填**，页面能力分类 |
| 预期列名 | `预期结果` | **`用例结果`**（cases 键仍为 `expected`） |
...
`fieldDefaults` 中 **不要** 写占位 `"用例结果": "0"`；由每条 case 的 `expected` 覆盖。
