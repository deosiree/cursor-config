#!/usr/bin/env bash
# 租户管理 UX：创建 → 搜索校验 → 删除 → 空列表校验（需已登录 session）。
...
# --- 步骤 7-10：创建后查询、删除、再查询确认 ---

log_step "7" "搜索「${TENANT_NAME}」，断言有且仅有 1 条（最多轮询 30s）"
assert_tenant_list_count "$TENANT_NAME" 1 || die "步骤7失败：创建后未在列表中找到 tenant0529，请确认是否创建成功"
