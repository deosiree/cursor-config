---
name: OpenCLI-通用路由
description: Use when 用户需要浏览器自动化 — 端到端测试 / 手动点击模拟 / SPA 爬虫 / 登录态复用 / 需 Chrome JS 渲染的场景。不要用在本机无 Chrome 或只需静态 HTML 时。
tags:
  - 浏览器自动化
  - OpenCLI
  - 自动化测试
...
> **定位：** 只做路由，不做拷贝。根据用户场景（自动化测试 / 手动点击 / 爬虫提取）分发到对应的已有 skill 或通用操作指南。
> **范围：** OpenCLI 的 `browser` 子命令能做的不只是测试 —— 它可以驱动真实 Chrome 执行任何浏览器操作，包括手动点击模拟、表单填写、SPA 爬虫、登录态复用等。
...
| 场景 | 条件 | 策略/引用 |
|------|------|----------|
| **自动化测试** | 用户要对某个页面进行「操作 → 断言」的端到端校验 | → [[references/场景-自动化测试.md]] |
| **手动点击自动化** | 用户要模拟人工操作：点按钮、填表单、切换 Tab、截图 | → [[references/场景-手动点击自动化.md]] |
| **爬虫与数据提取** | 用户要从 SPA / JS 渲染站点提取全文或结构化数据 | → [[references/场景-爬虫与数据提取.md]] |
| **登录会话管理** | 用户需要先登录再执行后续操作 | → [[references/场景-手动点击自动化.md#登录会话]]（共用） |
...
- **session 名称**：固定为 `nebula-ux` 跨 skill 复用（两个 test-skills 共用）
- **操作后截图**：重要步骤后截图，方便排查
...
| 只用 `click --name` 不兜底 | Element Plus 等组件库的按钮文本带空格/图标，role 定位失败 | 先用 `click --name`，失败后 `eval JS` 兜底（见 test-skills lib/common.sh） |
...
| 脚本里硬编码密码 | 不小心提交 git | 用 config JSON 文件 + `.gitignore` 本地覆盖（参考 test-skills 的 `config/ux-test.config.local.json`） |
...
| 被引用的已有 Skill | 角色 |
|-------------------|------|
| `test-skills/opencli-ux-role-tab-validation/` | 自动化测试：角色 Tab 校验跳转 |
| `test-skills/opencli-ux-tenant/` | 自动化测试：租户 CRUD 流程 |
