...
tags:
  ...
  - CRUD
should-trigger:
  - prompt 含 租户 + 创建 / 删除 / 搜索 / 清理 + OpenCLI
  - prompt 含 租户管理 + 自动化测试 / 回归 / E2E
  - prompt 含 tenant-create / tenant-verify / tenant-cleanup
  - prompt 含 登录 + 租户页 + OpenCLI
  - prompt 含 清理已存在的 tenant-xxx 租户
should-not-trigger:
  - 只需要手动指导操作租户页（不需要自动化执行）
  - 租户非主要目标（如「测一下角色弹窗」→ 走 role-tab-validation）
  - 没有 Chrome / opencli doctor 未通过
  - 纯 API 测试、不需要浏览器
---
