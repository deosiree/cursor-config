---
name: 自生长OpenCLI自动化
description: 自生长的OpenCLI自动化知识体系 — 底座路由中心。按场景分发到子领域 skill（租户管理、用户管理、角色Tab校验等），每次使用沉淀回自身避免知识散落。当需要浏览器自动化（端到端测试/手动点击/SPA爬虫/登录复用）时使用。
tags:
  - 浏览器自动化
  - OpenCLI
  - 自动化测试
...
| **自动化测试-API白名单表格E2E** | 菜单「白名单」弹窗、真实 API 插种、表格滚动条、50 条、sys-menu-whitelist-btn | → `opencli-ux-api-whitelist/`（直接路由） |
...
| `opencli-ux-api-whitelist/` | 自动化测试：菜单 API 白名单弹窗表格 E2E + 真实插种 + 滚动断言 |
...
# 4. auto-commit — 强制步骤。以下三步不得跳过。

# 4a. 执行 git commit
run_command:
  command: "git -C .cursor add -A && git commit -m \"🏗️ OpenCLI自生长: 新增场景「{name}」\""

# 4b. 验证 commit 是否实际写入
# 使用以下命令确认：如果 0 行输出 → auto-commit 未生效，必须 STOP 并向用户说明。
#   git -C .cursor log --oneline --grep="🏗️ OpenCLI自生长: 新增场景" -1
# 若未找到对应 commit，走 4c 退化路径，不得忽略。

# 4c. 无 git 环境的退化路径（仅在 4b 验证失败时执行）
# 如果 git 命令不存在 → write_file 创建 session-log/{date}-uncommitted-{name}.md
# 内容包含：未 commit 的文件列表 + 建议 commit 命令
# 然后向用户说明：「知识体系只做了磁盘写入，未进 git 版本管理。
# 下次进入该目录时请手动执行：git add . && git commit -m "🏗️ OpenCLI自生长: 新增场景「{name}」"」
```
...
创建完成后执行强制 commit 步骤（同 Step 3 #4a-#4c，commit message 前缀改为 `🌱`）：

```yaml
# 4a. git commit（子 skill 用 🌱 前缀）
run_command:
  command: "git -C .cursor add -A && git commit -m \"🌱 OpenCLI自生长: 新增子 skill「{name}」\""

# 4b. 验证 commit 存在
run_command:
  command: "git -C .cursor log --oneline --grep=\"🌱 OpenCLI自生长: 新增子 skill「{name}」\" -1"
# 如果输出为空 → STOP，走 4c 退化

# 4c. 无 git 退化：write_file 创建 session-log/{date}-uncommitted-skill-{name}.md
```
...
| **🔴 6** | **本次实跑结果是否已记录？** | → **追加到 `evals/实跑记录.tsv`**：`date<TAB>skill<TAB>result<TAB>env<TAB>notes`。PASS / FAIL / PARTIAL 都记，不要只记成功。|
