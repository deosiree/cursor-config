...
本路由中心引用但不拷贝以下已有 skill 的内容。所有引用路径**相对于本文件所在目录的上层**（即 `自生长的OpenCLI自动化知识体系/`）。
...
| opencli-ux-api-whitelist | `opencli-ux-api-whitelist/` | 自动化测试：菜单 API 白名单弹窗表格 E2E + 真实插种 + 滚动断言 | [[references/场景-自动化测试.md]] |
...
| 能力 | 文件 |
|------|------|
| 白名单 E2E 主脚本 | `scripts/test-api-whitelist-table-scroll.ps1` |
| 串行真实 API 插种 | `scripts/opencli-whitelist-seed-50-oneline.js` |
| 滚动断言（scrollbar wrap） | `scripts/opencli-whitelist-scroll-eval-oneline.js` |
| 踩坑与 E2E 说明 | `references/api-whitelist-table-e2e-pitfalls.md` |
| Element Plus 滚动模式 | `references/element-plus-table-scroll.md` |
| RED/GREEN 模板 | `template/before/常见失败.md`、`template/after/全流程通过.md` |
