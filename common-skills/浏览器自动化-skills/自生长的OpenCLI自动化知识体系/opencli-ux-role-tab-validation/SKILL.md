...
| 意图 | 动作 |
|:-----|:------|
| **路由守卫（先过这一步）** | → [`intention-skills/判断是否Tab校验场景/SKILL.md`](intention-skills/判断是否Tab校验场景/SKILL.md) — 确认需求确实是 Tab 校验 |
...
| 只跑某条用例 | → [`feature-skills/Tab校验执行/SKILL.md`](feature-skills/Tab校验执行/SKILL.md)（TC1/TC2/TC3/TC4 单用例） |
| 踩坑排查 | → [`references/role-tab-pitfalls.md`](references/role-tab-pitfalls.md) |
| 通过态验证 | → [`template/after/TC全通过.md`](template/after/TC全通过.md) |
...
| 问题 | 当前状态 | 目标 |
|:-----|:---------|:-----|
| intention-skills 偏少 | 只有 1 个路由守卫 | 可补充「评估是否可重试」节点 |
| 失败截图未在 SKILL 内定义路径 | 脚本写到了 `screenshots/`，但 SKILL 未显式声明 | 在输出契约中补充 `screenshotPath` |
| 父子 skill 的 Phase 编号未对齐 | 本 skill 无 Phase 表（父级有 Phase 0→4/0→3） | 对齐为 Phase 0→4（check→login→open→execute→assert） |
...
- 踩坑：`references/role-tab-pitfalls.md`
- 源码落点：[`useTabValidation.ts`](../../../apex_dev/src/composables/useTabValidation.ts)
- 主路由：[`自生长的OpenCLI自动化知识体系/SKILL.md`](../SKILL.md) → 路由到本 skill
- 公共模式：父级 [`references/公共模式与反模式.md`](../references/公共模式与反模式.md)（本 skill `lib/common.sh` 使用了 P2 click→eval 兜底、P6 el-overlay 弹窗模式）